     ...
     else
     {
         ...
         // Si aucun enregistrement n'est trouvé
          ...
          else
          {
					...
					$row = $connexion->prepare_request('SELECT etat_compte FROM users 
                           WHERE id=:id AND clef_activation=:clef', ['id'=>$_GET['numero_id'], 'clef'=>strtolower($_GET["clef"])]);
                    ...
                    {
                       $connexion->update('UPDATE users SET etat_compte=:etat WHERE id=:id AND clef_activation=:clef', array('etat'=>1, 'id'=>$_GET["numero_id"], 'clef'=>strtolower($_GET["clef"])));
    ...
    <!--Contenu: MENU VERTICAL version mobile, tablets et PC-->
	  ...
	  <div style="text-align: center!important; display: block">
          ...
          <div class="text-center">
              <a id="compte_active" role="button" href="Config/index.php" class="btn btn-primary" title="PAGE D'ACCUEIL "><span class="glyphicon glyphicon-home"></span> PAGE D'ACCUEIL</a>
          </div>
      </div>
		<?php
		}
		?>
         ...
         <div style="text-align: center!important; display: block">
             ...
             <div class="text-center">
                 <a role="button" href="Config/index.php" class="btn btn-primary" title="PAGE D'ACCUEIL"><span class="glyphicon glyphicon-home"></span> RETOURNER A LA PAGE D'ACCUEIL</a>
             </div>
         </div>
             <?php
         }
